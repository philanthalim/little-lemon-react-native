...
export function saveMenuItems(menuItems) {
  db.transaction((tx) => {
    const menuFormatted = menuItems.map((item) => {
      ...
      tx.executeSql(`INSERT INTO menuitems (uuid, title, price, category) VALUES ${menuFormatted}`);
  });
}
...
export async function filterByQueryAndCategories(query, activeCategories) {
  return new Promise((resolve, reject) => {
    ...
    if (query) {
      constrains.push(`title like ?`)
      params.push(`%${query}%`);
    }

    if (activeCategories.length > 0) {
      constrains.push(`category in (${activeCategories.map(() => '?').join(',')})`);
      params = [...params, ...activeCategories];
    }

    const constrainsStr = constrains.length > 0 ? 'WHERE ' + constrains.join(' and ') : '';

    console.log(constrainsStr)

    db.transaction((tx) => {
      tx.executeSql(`SELECT * FROM menuitems ${constrainsStr}`, params, (_, {rows}) => resolve(rows._array), reject);
    })
  });
}
